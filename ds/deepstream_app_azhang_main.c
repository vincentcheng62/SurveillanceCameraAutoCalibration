...
#include "deepstream_app_azhang.h"
...
typedef enum
{
    ...
    APP_CONFIG_ANALYTICS_YOLOV3_PGIE = 2,
...
#define PGIE_CLASS_ID_AEROPLANE 4
...
#define YOLOV3_PGIE_ID_CAR (2)
#define YOLOV3_PGIE_ID_BICYCLE (1)
#define YOLOV3_PGIE_ID_BIKE (3)
#define YOLOV3_PGIE_ID_PLANE (4)
#define YOLOV3_PGIE_ID_BUS (5)
#define YOLOV3_PGIE_ID_TRAIN (6)
#define YOLOV3_PGIE_ID_TRUCK (7)
#define YOLOV3_PGIE_ID_BOAT (8)
...
#define APP_TITLE "DeepStreamApp"
...
static AppConfigAnalyticsModel model_used = APP_CONFIG_ANALYTICS_YOLOV3_PGIE;
...
GOptionEntry entries[] = {
  ...
  {"pgie-model-used", 'm', 0, G_OPTION_ARG_INT, &model_used,
        "PGIE Model used; {0 - Unknown [DEFAULT]}, {1: Resnet 4-class [Car, Bicycle, Person, Roadsign]}, {2: YoloV3 80-class}",
      NULL}
  ,
  {NULL}
  ,
...
generate_ts_rfc3339 (char *buf, int buf_size)
{
  ...
  //g_snprintf (strmsec, sizeof (strmsec), ".%.6d", ms);
...
generate_event_msg_meta (gpointer data, gint class_id, gboolean useTs,
    ...
    NvDsObjectMeta * obj_params, double scaleW, double scaleH,
    NvDsFrameMeta* frame_meta, float frame_width, float frame_height
    )
{
  ...
  meta->ts_lc = (gchar *) g_malloc0 (MAX_TIME_STAMP_LEN + 1);

  /*Added custom attribute of frame*/
  meta->frameWidth = frame_width;
  meta->frameHeight = frame_height;
  /* Added custom attribute of average_fps over the entire run per stream */
  //meta->averageFps = fps_avg[meta->sensorId];
  meta->averageFps = fps_avg[meta->sensorId];

  /* Generate system timestamp */
  //time_t now = time(0);
  //struct tm *ltm = localtime(&now);
  //char buffer[MAX_TIME_STAMP_LEN];
  //setlocale(LC_ALL, "en_US");
  //strftime(buffer, MAX_TIME_STAMP_LEN, "%F %T", ltm);
  //meta->ts_lc = (gchar *) buffer;
  //g_snprintf(meta->ts_lc, sizeof(buffer),"%s", buffer);
  //meta->ts_lc = ctime(&now);
  time_t tloc;
  struct tm tm_log;
  struct timespec ts2;
  char strmsec[9];              //.nnnZ\0

  clock_gettime (CLOCK_REALTIME, &ts2);
  memcpy (&tloc, (void *) (&ts2.tv_sec), sizeof (time_t));
  localtime_r (&tloc, &tm_log);
  strftime (meta->ts_lc, MAX_TIME_STAMP_LEN, "%Y-%m-%d %H:%M:%S", &tm_log);
  int ms = ts2.tv_nsec / 1000000;
  g_snprintf (strmsec, sizeof (strmsec), ".%.6d", ms);
  strncat (meta->ts_lc, strmsec, MAX_TIME_STAMP_LEN);
  ...
  //g_print("Scaled bbox scalew=%f, scaleh=%f, left=%f, top=%f, width=%f, height=%f\n", scaleW, scaleH,
  //meta->bbox.left, meta->bbox.top, meta->bbox.width, meta->bbox.height);
  //g_print("event original left, top, width, height:%d,%d,%d,%d\n", obj_params->rect_params.left,
  //obj_params->rect_params.top, obj_params->rect_params.width, obj_params->rect_params.height);
   ...
   */
  /*
   * This is added for yolo v3 (class = 80) car class.
   */
  //if(model_used == APP_CONFIG_ANALYTICS_RESNET_PGIE_3SGIE_TYPE_COLOR_MAKE) {
  if (model_used == APP_CONFIG_ANALYTICS_YOLOV3_PGIE) {  
    ...
    if (class_id == PGIE_CLASS_ID_CAR) {
      ...
      meta->objType = NVDS_OBJECT_TYPE_CAR;
      //meta->objClassId = RESNET10_PGIE_3SGIE_TYPE_COLOR_MAKECLASS_ID_CAR;
      meta->objClassId = PGIE_CLASS_ID_CAR;
    } 
    else if (class_id == PGIE_CLASS_ID_BUS) {
      meta->type = NVDS_EVENT_MOVING;
      meta->objType = NVDS_OBJECT_TYPE_BUS;
      //meta->objClassId = RESNET10_PGIE_3SGIE_TYPE_COLOR_MAKECLASS_ID_CAR;
      meta->objClassId = PGIE_CLASS_ID_BUS;
    } 
    else if (class_id == PGIE_CLASS_ID_TRUCK) {
      meta->type = NVDS_EVENT_MOVING;
      meta->objType = NVDS_OBJECT_TYPE_TRUCK;
      //meta->objClassId = RESNET10_PGIE_3SGIE_TYPE_COLOR_MAKECLASS_ID_CAR;
      meta->objClassId = PGIE_CLASS_ID_TRUCK;
    } 
    else if (class_id == PGIE_CLASS_ID_BICYCLE) {
      meta->type = NVDS_EVENT_MOVING;
      meta->objType = NVDS_OBJECT_TYPE_BICYCLE;
      //meta->objClassId = RESNET10_PGIE_3SGIE_TYPE_COLOR_MAKECLASS_ID_CAR;
      meta->objClassId = PGIE_CLASS_ID_BICYCLE;
    } 
    else if (class_id == PGIE_CLASS_ID_MOTORBIKE) {
      meta->type = NVDS_EVENT_MOVING;
      meta->objType = NVDS_OBJECT_TYPE_MOTORBIKE;
      //meta->objClassId = RESNET10_PGIE_3SGIE_TYPE_COLOR_MAKECLASS_ID_CAR;
      meta->objClassId = PGIE_CLASS_ID_MOTORBIKE;
    } 
#ifdef GENERATE_DUMMY_META_EXT
  ...
  //g_print ("Object object id = %d w = %d h = %d c = %f\n", meta->trackingId, meta->bbox.width, 
  //meta->bbox.height, meta->confidence);
  
  /*
   * This is for ResNet (class = 4) vehicle class.
   */
  if (class_id == RESNET10_PGIE_3SGIE_TYPE_COLOR_MAKECLASS_ID_CAR) {
    if (class_id == RESNET10_PGIE_3SGIE_TYPE_COLOR_MAKECLASS_ID_CAR) {
      meta->type = NVDS_EVENT_MOVING;
    ...
    NvDsBatchMeta * batch_meta, guint index)
{
  ...
  for (NvDsMetaList * l_frame = batch_meta->frame_meta_list; l_frame != NULL;
   ...
   /***
    *  Add "Frame No.: XXX" on OSD display & output video
    */
    ...
    txt_params->x_offset = frame_meta->source_frame_width - 250;
  ...
  /***/
    ...
    for (l = frame_meta->obj_meta_list; l != NULL; l = l->next) {
      ...
      /*char a[MAX_DISPLAY_LEN];
      sprintf(a, "\n(%0.6f, %0.6f)", 1.123456, 1.123456);
      strncat (obj_meta->text_params.display_text, a, MAX_DISPLAY_LEN);*/

      {

        str_replace (obj_meta->text_params.display_text, obj_meta->text_params.display_text, "");
        ...
        scaleW =
            (double) frame_meta->source_frame_width /
            appCtx->config.streammux_config.pipeline_width;
        scaleH =
            (double) frame_meta->source_frame_height /
            appCtx->config.streammux_config.pipeline_height;

        //g_print("scaleW, scaleH: %f, %f\n", scaleW, scaleH);

        //g_print("source_frame_width=%d, pipeline_width=%d\n", frame_meta->source_frame_width,
        //appCtx->config.streammux_config.pipeline_width);
        ...
        //Added frame size pass to event metadata
        //float frame_width = appCtx->config.tiled_display_config.width;
        //float frame_height = appCtx->config.tiled_display_config.height;
        gint frame_width = frame_meta->source_frame_width;
        gint frame_height = frame_meta->source_frame_height;

        //g_print ("Object class id = %d text = %s\n", obj_meta->class_id, obj_meta->text_params.display_text);
        ...
        generate_event_msg_meta (msg_meta, obj_meta->class_id, TRUE,
            ...
            frame_meta, frame_width, frame_height);
    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
      l_frame = l_frame->next) {
        ...
        for (l_obj = frame_meta->obj_meta_list; l_obj != NULL;
            ...
            if (obj_meta->class_id == YOLOV3_PGIE_ID_BICYCLE || obj_meta->class_id == YOLOV3_PGIE_ID_CAR || obj_meta->class_id == YOLOV3_PGIE_ID_BIKE
                || obj_meta->class_id == YOLOV3_PGIE_ID_PLANE || obj_meta->class_id == YOLOV3_PGIE_ID_BUS || obj_meta->class_id == YOLOV3_PGIE_ID_TRAIN
                || obj_meta->class_id == YOLOV3_PGIE_ID_TRUCK || obj_meta->class_id == YOLOV3_PGIE_ID_BOAT) {
                vehicle_count++;
                total_count++;
            }
            else {
        ...
        offset = snprintf(display_meta->text_params[0].display_text, MAX_DISPLAY_LEN, "Vehicle Count = %d ", vehicle_count);
        //offset = snprintf(display_meta->text_params[0]->display_text, MAX_DISPLAY_LEN, "Object Count = %d ", total_count);
        //offset = snprintf(display_meta->text_params[0]->display_text + offset , MAX_DISPLAY_LEN, "Vehicle Count = %d ", vehicle_count);
...
/**
 * Callback function to be called once all inferences (Primary + Secondary) and tracker
 * are done. This is opportunity to modify content of the metadata.
 * e.g. Here "person xxx" is being replaced with "" and corresponding counts
 * can maintained. It should be modified according to network classes
 * or can be removed altogether if not required.
 */
static void
  ...
  for (NvDsMetaList * l_frame = batch_meta->frame_meta_list; l_frame != NULL;
    ...
    for (NvDsMetaList * l_obj = frame_meta->obj_meta_list; l_obj != NULL;
      ...
      if (obj->unique_component_id ==
        ...
        // Not to display bbouding box and text for all non-vehicle class objects.
        // non-vehicle class id = 0, >=9.
        // set alpha = 0.0 to make color transparent.
        if (obj->class_id < 0 || obj->class_id >= 9) { //include 0 to include person object
            str_replace (obj->text_params.display_text, obj->text_params.display_text, "");
            ...
            obj->text_params.text_bg_clr.red = 0.0;
            obj->text_params.text_bg_clr.green = 0.0;
            obj->text_params.text_bg_clr.blue = 0.0;
            obj->text_params.text_bg_clr.alpha = 0.0;
          }
        else
        {
            num_vehicle++;           

            double bottom_pt_x = (double) obj->rect_params.left + obj->rect_params.width / 2;
            double bottom_pt_y = (double) obj->rect_params.top + obj->rect_params.height; 
            //g_print("Original bbox left, top, width, height: %d, %d, %d, %d\n", obj->rect_params.left, obj->rect_params.top, 
            //obj->rect_params.width, obj->rect_params.height);
            //g_print("ScaleW, ScaleH: %f, %f\n", scaleW, scaleH);

            /*double hmgphy[] = { -2.6273577895047262e-03, 2.8526694368399907e-01,
                 2.2426324947789926e+01, -1.3384762195828952e-02,
                 1.4529377038342806e+00, 1.1420984254043115e+02,
                 -1.1720986825562812e-04, 1.2721245230480245e-02, 1. };*/
             //specifically for the garage gate scene 4k video from Xi'an Huawei
            /*double hmgphy[] = {
            1.0098536050350864e-02, -1.8872234018272865e-01,
            3.4120743301383428e+01, 3.2111002278906300e-02,
            -6.0009393547701106e-01, 1.0849544949859926e+02,
            2.9596497101075971e-04, -5.5310370344499203e-03, 1. };*/
            //scaled for garage gate scene 1920*1080 video from Xi'an Huawei
            /*double hmgphy[] = {
              2.0197057992478149e-02, -3.7744468431150430e-01,
              3.4120743300932979e+01, 6.4221959696715458e-02,
              -1.2001878834985145e+00, 1.0849544949599871e+02,
              5.9192952853099657e-04, -1.1062074184557662e-02, 1. 
            };*/
            //2.13,2 scaled homography
            double hmgphy[] = {
               2.1513937400486301e-02, -3.7748908400169978e-01,
               3.4120743345816308e+01, 6.8409330779082997e-02,
              -1.2003290642188540e+00, 1.0849544945831133e+02,
               6.3052424919565124e-04, -1.1063375440396910e-02, 1.
            };  

	          CvMat* hmgphy_matrix = cvCreateMat(3, 3, CV_64FC1);
	          cvInitMatHeader(hmgphy_matrix, 3, 3, CV_64FC1, hmgphy, CV_AUTOSTEP);
	
	          //double bottmo_pt[] = { 1804.0, 539.0 };
            //double bottmo_pt[] = {1403.6842041015625, 579.0789794921875};
            double bottmo_pt[] = {bottom_pt_x, bottom_pt_y};
	          CvMat* src = cvCreateMat(1, 2, CV_64FC2);
	          cvInitMatHeader(src, 1, 2, CV_64FC2, bottmo_pt, CV_AUTOSTEP);

	          CvMat* dst = cvCreateMat(1, 2, CV_64FC2);

	          cvPerspectiveTransform(src, dst, hmgphy_matrix);
	
            char gps[MAX_DISPLAY_LEN];
            sprintf(gps, "\n(%0.6f, %0.6f)", *((double*)(dst->data.ptr)), *((double*)(dst->data.ptr)+1));
            strncat (obj->text_params.display_text, gps, MAX_DISPLAY_LEN);
            ////str_replace (obj->text_params.display_text, obj->text_params.display_text, gps);
            //g_print("bottom_middle=[%f, %f], lat=%0.6f, lon=%0.6f\n", bottom_pt_x, bottom_pt_y, *((double*)(dst->data.ptr)), *((double*)(dst->data.ptr)+1));
            //g_print("bottom_middle=[%f, %f], lat=%0.6f, lon=%0.6f\n", 1403.6842041015625, 579.0789794921875, *((double*)(dst->data.ptr)), *((double*)(dst->data.ptr)+1));


            //g_print("x=%0.15f, y=%0.15f\n", *((double*)(dst->data.ptr)), *((double*)(dst->data.ptr)+1));
            

            obj->rect_params.border_color.alpha = 1.0;
            obj->text_params.font_params.font_size = 12; 
            obj->text_params.text_bg_clr.alpha = 0.2;
            obj->text_params.font_params.font_color.alpha = 1.0;
            switch (obj->class_id) {
              case 0:
                obj->text_params.text_bg_clr.red = 1.0;
                obj->text_params.text_bg_clr.green = 0.4;
                obj->text_params.text_bg_clr.blue = 0.0;
                //obj->text_params.text_bg_clr.alpha = 0.0;
                break;              
              case 1:
              ...
              case 2:
                obj->text_params.text_bg_clr.red = 1.0;
                obj->text_params.text_bg_clr.green = 1.0;
                obj->text_params.text_bg_clr.blue = 0.0;
                //obj->text_params.text_bg_clr.alpha = 0.0;
                break;
              case 3:
                obj->text_params.text_bg_clr.red = 0.0;
                obj->text_params.text_bg_clr.green = 0.0;
                obj->text_params.text_bg_clr.blue = 1.0;
                //obj->text_params.text_bg_clr.alpha = 0.0;
                break;
              case 5:
                obj->text_params.text_bg_clr.red = 1.0;
...
main (int argc, char *argv[])
{
  ...
  ctx = g_option_context_new ("Nvidia DeepStream App");
  ...
  for (i = 0; i < num_instances; i++) {
    ...
    if (!create_pipeline (appCtx[i], bbox_generated_probe_after_analytics,
      ...
      /*if (!create_pipeline (appCtx[i], NULL,
            bbox_generated_probe_after_analytics, perf_cb, overlay_graphics)) {*/
