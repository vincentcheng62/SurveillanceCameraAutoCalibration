...
videopath = 'test_far_ppl_9_mount.mp4'
...
while(True):
    ...
    if IsUsingZoomInDetection:
        ...
        zoom_img = np.array(zoom_pilimg)
        zoom_pad_x = max(zoom_img.shape[0] - zoom_img.shape[1], 0) * (img_size / max(zoom_img.shape))
        zoom_pad_y = max(zoom_img.shape[1] - zoom_img.shape[0], 0) * (img_size / max(zoom_img.shape))
        zoom_unpad_h = img_size - zoom_pad_y
        zoom_unpad_w = img_size - zoom_pad_x        
    ...
    if IsUsingZoomInDetection:
        if zoom_detections is not None:
            ...
            start = time.time()
            tracked_objects = zoom_mot_tracker.update(zoom_detections.cpu())

            unique_labels = zoom_detections[:, -1].cpu().unique()
            n_cls_preds = len(unique_labels)
            print("tracker update took {} seconds.".format(time.time() - start))
            ...
            for x1, y1, x2, y2, obj_id, cls_pred in tracked_objects:
                ...
                starto = time.time()
                box_h = int(((y2 - y1) / zoom_unpad_h) * zoom_img.shape[0])
                box_w = int(((x2 - x1) / zoom_unpad_w) * zoom_img.shape[1])
                y1 = int(((y1 - zoom_pad_y // 2) / zoom_unpad_h) * zoom_img.shape[0])
                x1 = int(((x1 - zoom_pad_x // 2) / zoom_unpad_w) * zoom_img.shape[1])
                bb=(x1, y1, box_w, box_h)
                color = colors[int(obj_id) % len(colors)]
                cls = classes[int(cls_pred)]

                if IsOutputVideo:
                    cv2.rectangle(zoom_frame, (x1, y1), (x1+box_w, y1+box_h), color, 4)
