...
if __name__ == '__main__':
    ...
    initialguess = np.zeros((3, 3), np.float32)
    initialguess[0, 0] = 1942.2
    #camera_matrix_manual[1, 0] = 0
    #camera_matrix_manual[2, 0] = 0
    #camera_matrix_manual[0, 1] = 0
    initialguess[1, 1] = 1942.2
    #camera_matrix_manual[2, 1] = 0
    initialguess[0, 2] = 1920*0.5
    initialguess[1, 2] = 1080*0.5
    initialguess[2, 2] = 1
    ...
    #rms, camera_matrix, dist_coefs, rvecs, tvecs = cv.calibrateCamera(obj_points, img_points, (w, h), initialguess, None, flags=cv.CALIB_FIX_PRINCIPAL_POINT + cv.CALIB_FIX_ASPECT_RATIO+cv.CALIB_RATIONAL_MODEL+cv.CALIB_USE_INTRINSIC_GUESS)
    #rms, camera_matrix, dist_coefs, rvecs, tvecs = cv.calibrateCamera(obj_points, img_points, (w, h), None, None, flags=cv.CALIB_ZERO_TANGENT_DIST+cv.CALIB_FIX_K1+cv.CALIB_FIX_K2+cv.CALIB_FIX_K3+cv.CALIB_FIX_K4+cv.CALIB_FIX_K5+cv.CALIB_FIX_K6)
    rms, camera_matrix, dist_coefs, rvecs, tvecs = cv.calibrateCamera(obj_points, img_points, (w, h), None, None, flags=cv.CALIB_FIX_K3)
